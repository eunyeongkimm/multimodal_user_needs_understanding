"""v5_chunk_plan.py로 나눈 청크(has_complaint=False 11,127건)를 순차적으로
OpenAI Batch API에 제출/확인한다. batch1_check.py/v3_check.py/v4_check.py와
동일한 재시도/스킵/재시작-중복방지/일시적오류 처리 로직 적용.

모든 청크가 터미널 상태에 도달하면 완료를 알리고, v5_autorun.sh가 이어서
merge + 3버전 비교표 출력을 자동 실행한다.
"""
...
from v5_prompt import make_prompt, normalize_gpt_label

BASE_DIR = Path(__file__).resolve().parent.parent
PLAN_PATH = BASE_DIR / "outputs" / "v5_chunks_plan.json"
STATE_PATH = BASE_DIR / "outputs" / "v5_chunks_state.json"
CALL_TEXTS_PATH = BASE_DIR / "outputs" / "batch1_call_texts.parquet"
PARTIAL_DIR = BASE_DIR / "outputs" / "v5_partial"
FINAL_PATH = BASE_DIR / "outputs" / "gold_actual_batch1_v5.parquet"

MODEL = "gpt-4.1-mini"
PROMPT_VERSION = "v5"
PROJECT_TAG = "gold_actual_batch1_v5"
MAX_RETRIES = 2
...
def submit_chunk(client, chunk_index: int, call_texts: pd.DataFrame):
    ...
    jsonl_path = BASE_DIR / "outputs" / f"v5_chunk{chunk_index}_requests.jsonl"
...
def retry_failed_requests(client, chunk_index: int, failed_records: list, call_texts: pd.DataFrame,
    ...
    for custom_id, err_msg in failed_records:
        ...
        if text_row.empty:
            ...
            rows.append({"call_id": custom_id, "label_v5": None})
            continue
        ...
        rows.append({"call_id": custom_id, "label_v5": label})
...
def download_chunk_result(client, chunk_index: int, batch_id: str, call_texts: pd.DataFrame):
    ...
    for line in content.strip().split("\n"):
        ...
        try:
            ...
            label = None
        rows.append({"call_id": call_id, "label_v5": label})
    df = pd.DataFrame(rows)

    # batch 자체는 completed로 보고돼도, 개별 요청이 API 레벨에서 실패(예: 500)하면
    # output_file_id가 아니라 error_file_id에 기록되고 request_counts.failed에도
    # 반영 안 될 수 있다 (batch1/v2에서 이렇게 1건이 조용히 유실된 적 있음).
    # 여기서 error_file_id를 확인해 그런 건을 잡아내고 즉시 동기 API로 재시도한다.
...
def _run(client):
    ...
    if all(s in terminal for s in statuses):
        ...
        final_df = (pd.concat(parts, ignore_index=True) if parts
                    else pd.DataFrame(columns=["call_id", "label_v5", "prompt_version", "model"]))
        ...
        n_null = final_df["label_v5"].isna().sum() if len(final_df) else 0
        ...
        if len(final_df):
            print("\n라벨 분포:")
            print(final_df["label_v5"].value_counts(dropna=False).to_string())
