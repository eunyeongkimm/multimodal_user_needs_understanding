...
작업 지시의 **"기존 핵심실험 800콜(불만제기 250 오버샘플)"** 에 정확히 맞는
세트가 저장소에 없다. 현재 존재하는 것은:

| 세트 | 콜 수 | 불만제기 | 오디오 |
|---|---|---|---|
| `model_pilot_sample.csv` | 250 | 50 (오버샘플) | **추출 완료** (`outputs/audio_seg/`, Drive 업로드됨) |
| `v_unified_sample_call_ids.csv` | 2,000 | **250 (오버샘플)** | 미추출 |

"불만제기 250 오버샘플"은 **2,000콜 세트와 일치**한다(800이 아니라 2,000).
800콜 세트를 따로 갖고 계시면 `PROBE_ROOT`만 그쪽으로 바꾸면 된다.

- **250콜로 지금 바로 돌릴 수 있다** — 오디오가 이미 Drive에 있다. 단
  불만 50건 / 특징 차원 1280~3584이라 **p ≫ n**이고, AUC 절대값은
  불안정하다(층 간 *상대* 비교는 n·p가 모든 층에서 같아 공정하다).
- **2,000콜(또는 800콜)** 을 쓰려면 맥에서 먼저:
  `python scripts/probe_audio_prep.py --calls unified2k`
  → `outputs/probe_set_unified2k/` 를 Drive에 올린다. n이 커져 AUC가 안정된다.

아래 `PROBE_ROOT` / `USE_LEGACY_250` 스위치로 선택한다.
""")
...
if USE_LEGACY_250:
    ...
    labels['gender'] = None     # 250 세트에는 성별이 없다 -> positive control은 셀 10에서 처리
