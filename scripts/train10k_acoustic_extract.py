"""train10k(게이트 프로토타입 B) N=2 pool의 발화별 acoustic feature 추출.
stage2b_acoustic_extract.py와 동일 로직(F0, Energy dB/amp, silence ratio),
대상 pool만 train10k_utterance_pool.parquet(26,440행)으로 교체.
외장하드(AIHub) 마운트 필요.

체크포인트: 청크 단위로 나눠 처리, 이미 완료된 청크는 재실행 시 건너뜀.
"""
...
PARTIAL_DIR = BASE_DIR / "outputs" / "train10k_acoustic_partial"
PROGRESS_PATH = BASE_DIR / "outputs" / "train10k_acoustic_progress.json"
FINAL_PATH = BASE_DIR / "outputs" / "train10k_acoustic_features.parquet"
AUDIO_BASE_DIR = Path(
...
def main():
    if not AUDIO_BASE_DIR.exists():
        print(f"ERROR: 외장하드 미마운트 - {AUDIO_BASE_DIR} 없음. 연결 후 재실행하세요.")
        return
    ...
    if len(done_chunks) == n_chunks:
        ...
        print("TRAIN10K_ACOUSTIC_COMPLETE")
