"""v3 vs v3+게이트(강/중/약) Batch 제출/확인. <카테고리> 태그 파싱(번호prefix
정규화 적용, 최근접매칭 없음). error_file_id 패치 적용."""
...
STATE_PATH = BASE_DIR / "outputs" / "gate_prompt_chunks_state.json"
PARTIAL_DIR = BASE_DIR / "outputs" / "gate_prompt_partial"
FINAL_PATH = BASE_DIR / "outputs" / "gate_prompt_gpt_predictions.parquet"

MODEL = "gpt-4.1-mini"
PROJECT_TAG = "gate_prompt"
MAX_RETRIES = 2
...
def submit_chunk(client, chunk_index: int, plan: list, prompts: pd.Series):
    ...
    jsonl_path = BASE_DIR / "outputs" / f"gate_prompt_chunk{chunk_index}_requests.jsonl"
...
def retry_failed_requests(client, failed_records: list, max_retries: int = 3) -> pd.DataFrame:
...
def download_chunk_result(client, chunk_index: int, batch_id: str):
    ...
    if batch.error_file_id:
        ...
        if failed_records:
            print(f"[error_file_id 패치 작동] 청크 {chunk_index} 개별 요청 실패 {len(failed_records)}건: "
                  f"{[c for c, _ in failed_records]}")
            retry_df = retry_failed_requests(client, failed_records)
            n_recovered = retry_df["predicted_label"].notna().sum()
            print(f"  -> 동기 재시도로 {n_recovered}/{len(failed_records)}건 복구")
...
def _run(client):
    ...
    if all(s in terminal for s in statuses):
        ...
        req_idx = pd.read_parquet(REQUEST_INDEX_PATH)[["custom_id", "call_id", "variant", "gate_level", "gold_actual"]]
        ...
        print(merged.groupby("variant")["parse_error"].apply(lambda s: s.notna().sum()))
        if n_failed:
