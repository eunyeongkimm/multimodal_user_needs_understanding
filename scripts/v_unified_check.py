"""v_unified 층화샘플 2,000건 Batch 제출/확인. error_file_id 패치 적용."""
...
from v_unified_prompt import normalize_gpt_label
...
STATE_PATH = BASE_DIR / "outputs" / "v_unified_chunks_state.json"
PARTIAL_DIR = BASE_DIR / "outputs" / "v_unified_partial"
FINAL_PATH = BASE_DIR / "outputs" / "v_unified_sample_labels.parquet"

MODEL = "gpt-4.1-mini"
PROJECT_TAG = "v_unified"
MAX_RETRIES = 2
...
def submit_chunk(client, chunk_index: int, plan: list, prompts: pd.Series):
    ...
    jsonl_path = BASE_DIR / "outputs" / f"v_unified_chunk{chunk_index}_requests.jsonl"
...
def _run(client):
    ...
    if all(s in terminal for s in statuses):
        ...
        req_idx = pd.read_parquet(REQUEST_INDEX_PATH)[["custom_id", "call_id", "gold_actual"]]
