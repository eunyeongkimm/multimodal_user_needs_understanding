"""train10k v4(항의 게이트) Batch 제출/확인. error_file_id 패치 적용.
출력: outputs/train10k_gpt_labels_v4.parquet (call_id, label_v4, has_complaint)
"""
...
from v4_prompt import parse_v4_response
...
STATE_PATH = BASE_DIR / "outputs" / "train10k_v4_chunks_state.json"
PARTIAL_DIR = BASE_DIR / "outputs" / "train10k_v4_partial"
FINAL_PATH = BASE_DIR / "outputs" / "train10k_gpt_labels_v4.parquet"

MODEL = "gpt-4.1-mini"
PROJECT_TAG = "train10k_v4"
MAX_RETRIES = 2
...
def submit_chunk(client, chunk_index: int, plan: list, prompts: pd.Series):
    ...
    jsonl_path = BASE_DIR / "outputs" / f"train10k_v4_chunk{chunk_index}_requests.jsonl"
...
def retry_failed_requests(client, failed_records: list, max_retries: int = 3) -> pd.DataFrame:
    ...
    for custom_id, err_msg in failed_records:
        if custom_id not in prompts.index:
            rows.append({"custom_id": custom_id, "has_complaint": None, "label_v4": None})
            continue
        ...
        if has_complaint is None and last_err is not None:
            print(f"  경고: {custom_id} 동기 재시도도 실패 (원래 에러: {err_msg}, 재시도 에러: {last_err})")
        rows.append({"custom_id": custom_id, "has_complaint": has_complaint, "label_v4": label})
...
def _run(client):
    ...
    if all(s in terminal for s in statuses):
        ...
        preds = pd.concat(parts, ignore_index=True) if parts else pd.DataFrame(
            columns=["custom_id", "has_complaint", "label_v4"])
        req_idx = pd.read_parquet(REQUEST_INDEX_PATH)[["custom_id", "call_id"]]
        merged = req_idx.merge(preds, on="custom_id", how="left")
        merged.to_parquet(FINAL_PATH, index=False)
        n_null = merged["label_v4"].isna().sum()
        print(f"\n저장: {FINAL_PATH} ({len(merged)}행)")
        print(f"파싱 실패/누락: {n_null}건")
        n_complaint = (merged["has_complaint"] == True).sum()  # noqa: E712
        print(f"has_complaint=True (불만제기 확정): {n_complaint}건")
        if n_failed:
