"""B/D x N{2,3,5} v3_orig(reasoning) 재실행 Batch 제출/확인.
<카테고리> 태그 파싱(번호prefix는 normalize_gpt_label로 정규화, 최근접매칭 없음).
형식오류/태그누락은 parse_error에 기록하고 predicted_label=None으로 두되,
최종 분석(finalize) 단계에서 "오답"으로 처리한다(여기서는 제외하지 않고 그대로 저장).
"""
...
STATE_PATH = BASE_DIR / "outputs" / "stage2_bd_v3_chunks_state.json"
PARTIAL_DIR = BASE_DIR / "outputs" / "stage2_bd_v3_partial"
FINAL_PATH = BASE_DIR / "outputs" / "stage2_bd_v3_gpt_predictions.parquet"
PROGRESS_PATH = BASE_DIR / "outputs" / "stage2_progress.json"

MODEL = "gpt-4.1-mini"
PROJECT_TAG = "stage2_bd_v3_reasoning"
MAX_RETRIES = 2
...
def parse_response(raw: str):
    ...
    if not m:
        return None, "형식오류(카테고리태그누락)"
    cat = normalize_gpt_label(m.group(1).strip())
    if cat not in CATEGORIES:
        return None, f"형식오류(카테고리불일치:{cat!r})"
    return cat, None
...
def submit_chunk(client, chunk_index: int, plan: list, prompts: pd.Series):
    ...
    jsonl_path = BASE_DIR / "outputs" / f"stage2_bd_v3_chunk{chunk_index}_requests.jsonl"
...
def retry_failed_requests(client, chunk_index: int, failed_records: list, max_retries: int = 3) -> pd.DataFrame:
    ...
    for custom_id, err_msg in failed_records:
        if custom_id not in prompts.index:
            rows.append({"custom_id": custom_id, "predicted_label": None, "parse_error": "프롬프트없음"})
            continue
        prompt = prompts.loc[custom_id]
        label, perr, last_err = None, None, None
        for attempt in range(max_retries):
            try:
                resp = client.chat.completions.create(
                    model=MODEL, messages=[{"role": "user", "content": prompt}], temperature=0,
                )
                label, perr = parse_response(resp.choices[0].message.content)
                break
        ...
        rows.append({"custom_id": custom_id, "predicted_label": label, "parse_error": perr})
...
def _run(client):
    ...
    if all(s in terminal for s in statuses):
        ...
        preds = pd.concat(parts, ignore_index=True) if parts else pd.DataFrame(
            columns=["custom_id", "predicted_label", "parse_error"])
        req_idx = pd.read_parquet(REQUEST_INDEX_PATH)[["custom_id", "call_id", "condition", "n", "window_version"]]
        ...
        print(f"파싱 실패/누락: {n_null}건 ({n_null/len(merged)*100:.2f}%)")
        print(merged.groupby("parse_error", dropna=False).size().sort_values(ascending=False).head(10))
        if n_failed:
            mark_progress("bd_v3_reasoning", {"status": "done_with_failures", "failed_permanent": n_failed, "total": len(chunks)})
            print("ALL_CHUNKS_DONE_WITH_FAILURES")
        else:
            mark_progress("bd_v3_reasoning", {"status": "done", "total": len(chunks)})
            print("\nALL_CHUNKS_COMPLETE")
