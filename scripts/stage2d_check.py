...
STATE_PATH = BASE_DIR / "outputs" / "stage2d_chunks_state.json"
PARTIAL_DIR = BASE_DIR / "outputs" / "stage2d_partial"
FINAL_PATH = BASE_DIR / "outputs" / "stage2d_gpt_predictions.parquet"
...
MODEL = "gpt-4.1-mini"
PROMPT_VERSION = "stage2d"
PROJECT_TAG = "stage2d_surface_labels"
MAX_RETRIES = 2
...
def build_jsonl(chunk_index: int, custom_ids: list, prompts: pd.Series) -> Path:
    jsonl_path = BASE_DIR / "outputs" / f"stage2d_chunk{chunk_index}_requests.jsonl"
...
def retry_failed_requests(client, chunk_index: int, failed_records: list, max_retries: int = 3) -> pd.DataFrame:
    ...
    프롬프트는 stage2d_request_index.parquet에서 custom_id로 바로 조회한다."""
...
def _run(client):
    ...
    if all(s in terminal for s in statuses):
        ...
        if n_failed:
            ...
            mark_progress("gpt_conditions", {"status": "done_with_failures", "failed_permanent": n_failed,
                                              "total": len(chunks)})
            print("ALL_CHUNKS_DONE_WITH_FAILURES")
        else:
            mark_progress("gpt_conditions", {"status": "done", "total": len(chunks)})
            print("\nALL_CHUNKS_COMPLETE")
