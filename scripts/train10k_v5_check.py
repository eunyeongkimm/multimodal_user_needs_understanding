"""train10k v5(순수 6종 재분류) Batch 제출/확인. error_file_id 패치 적용.
출력: outputs/train10k_gpt_labels_v5.parquet (call_id, label_v5)
"""
...
from v5_prompt import normalize_gpt_label
...
STATE_PATH = BASE_DIR / "outputs" / "train10k_v5_chunks_state.json"
PARTIAL_DIR = BASE_DIR / "outputs" / "train10k_v5_partial"
FINAL_PATH = BASE_DIR / "outputs" / "train10k_gpt_labels_v5.parquet"

MODEL = "gpt-4.1-mini"
PROJECT_TAG = "train10k_v5"
MAX_RETRIES = 2
...
def submit_chunk(client, chunk_index: int, plan: list, prompts: pd.Series):
    ...
    jsonl_path = BASE_DIR / "outputs" / f"train10k_v5_chunk{chunk_index}_requests.jsonl"
...
def retry_failed_requests(client, failed_records: list, max_retries: int = 3) -> pd.DataFrame:
    ...
    for custom_id, err_msg in failed_records:
        if custom_id not in prompts.index:
            rows.append({"custom_id": custom_id, "label_v5": None})
            continue
        ...
        rows.append({"custom_id": custom_id, "label_v5": label})
...
def download_chunk_result(client, chunk_index: int, batch_id: str):
    ...
    if batch.error_file_id:
        ...
        if failed_records:
            ...
            n_recovered = retry_df["label_v5"].notna().sum()
...
def _run(client):
    ...
    if all(s in terminal for s in statuses):
        ...
        preds = pd.concat(parts, ignore_index=True) if parts else pd.DataFrame(columns=["custom_id", "label_v5"])
        ...
        n_null = merged["label_v5"].isna().sum()
