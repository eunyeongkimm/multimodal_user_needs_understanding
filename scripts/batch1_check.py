"""batch1_chunks_plan.py로 나눈 19개 청크를 순차적으로 OpenAI Batch API에 제출/확인한다.

동작 방식 (한 번 실행할 때마다):
0. 로컬 state가 유실/오래됐을 가능성에 대비해, pending 청크는 서버 배치 목록과
   대조해 이미 제출된 것이 있으면 그걸 재사용한다 (재시작 시 중복 제출 방지).
1. 상태 파일(outputs/batch1_chunks_state.json)이 없으면 초기화하고 청크 0을 제출.
2. "submitted" 상태인 청크가 있으면 상태를 조회:
   - completed: 결과를 outputs/batch1_partial/chunk_{i}.parquet로 저장하고,
...
3. 모든 청크가 completed/failed_permanent(터미널 상태)면 completed된 것만 병합해
   outputs/gold_actual_batch1.parquet로 저장한다. 실패한 청크가 하나도 없으면
   "ALL_CHUNKS_COMPLETE", 일부라도 영구 실패했으면 "ALL_CHUNKS_DONE_WITH_FAILURES"를
   출력한다 (두 경우 모두 더 이상 자동으로 할 일이 없다는 뜻 - 후자는 사람 확인 필요).

조직의 gpt-4.1-mini Batch 큐 토큰 한도(2,000,000) 때문에 한 번에 하나의 청크만
"in-flight" 상태로 유지한다.
"""
...
STATE_PATH = BASE_DIR / "outputs" / "batch1_chunks_state.json"
CALL_TEXTS_PATH = BASE_DIR / "outputs" / "batch1_call_texts.parquet"
PARTIAL_DIR = BASE_DIR / "outputs" / "batch1_partial"
FINAL_PATH = BASE_DIR / "outputs" / "gold_actual_batch1.parquet"

MODEL = "gpt-4.1-mini"
PROMPT_VERSION = "v2"
MAX_RETRIES = 2  # 청크 실패 시 최대 재시도 횟수 (총 시도 = 1 + MAX_RETRIES). 소진되면 그 청크만 건너뛰고 계속 진행.
...
def reconcile_with_server(client, state):
    """로컬 state가 유실/오래된 경우에도 서버에 이미 제출된 배치를 중복
    제출하지 않도록, pending으로 표시된 청크 중 실제로는 서버에 이미
    존재하는 배치가 있는지 조회해서 상태를 맞춘다 (재시작 안전장치)."""
    ...
    for b in client.batches.list(limit=100):
        meta = b.metadata or {}
        if meta.get("project") != "gold_actual_batch1" or "chunk_index" not in meta:
            continue
...
def submit_chunk(client, chunk_index: int, call_texts: pd.DataFrame):
    ...
    jsonl_path = BASE_DIR / "outputs" / f"batch1_chunk{chunk_index}_requests.jsonl"
    ...
    batch = client.batches.create(
        ...
        metadata={"project": "gold_actual_batch1", "chunk_index": str(chunk_index), "prompt_version": PROMPT_VERSION},
    )
...
def retry_failed_requests(client, chunk_index: int, failed_records: list, call_texts: pd.DataFrame,
    ...
    배경: batch1(v2) 처리 중 청크4에서 J18_S002691 1건이 OpenAI 쪽 500 에러로
    error_file_id에만 기록되고 output_file_id에는 없어서, batch 자체는
    'completed'(failed=0)로 보고됐음에도 조용히 결과에서 누락된 사례가 있었음.
    """
    rows = []
    for custom_id, err_msg in failed_records:
        ...
        if text_row.empty:
            ...
            rows.append({"call_id": custom_id, "label": None})
            continue
        ...
        rows.append({"call_id": custom_id, "label": label})
...
def download_chunk_result(client, chunk_index: int, batch_id: str, call_texts: pd.DataFrame):
    ...
    if batch.error_file_id:
        ...
        if failed_records:
            ...
            n_recovered = retry_df["label"].notna().sum()
...
def main():
    ...
    # 네트워크/서버 쪽 일시적 오류는 버그가 아니라 재시도로 해결되는 문제이므로
    # 여기서 조용히 삼키고 다음 폴링에 다시 시도한다 (autorun 루프를 멈추지 않기 위함).
    # 그 외 예외(코드 버그 등)는 그대로 올려서 Traceback을 출력 - autorun이 감지해 멈춘다.
    try:
        _run(client)
...
def _run(client):
    ...
    if in_flight is None:
        ...
        if pending is not None:
            ...
            return
        # pending도 없고 in_flight도 없으면 전부 completed (또는 failed) 상태
    else:
        ...
        if batch.status == "completed":
            ...
            return  # 아직 진행 중, 다음 체크 때 다시 확인

    # 모든 청크가 터미널 상태(completed 또는 failed_permanent)인지 확인
