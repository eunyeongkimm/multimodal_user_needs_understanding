...
STATE_PATH = BASE_DIR / "outputs" / "stage2m_chunks_state.json"
PARTIAL_DIR = BASE_DIR / "outputs" / "stage2m_partial"
FINAL_PATH = BASE_DIR / "outputs" / "stage2m_gpt_predictions_BD.parquet"
...
MODEL = "gpt-4.1-mini"
PROMPT_VERSION = "stage2m_gender_renorm"
PROJECT_TAG = "stage2m_gender_renorm_BD"
MAX_RETRIES = 2
TOKEN_CEILING = 1_600_000  # 동시 in-flight 총 토큰 상한 (조직 한도 200만 대비 안전마진)
...
def build_jsonl(chunk_index: int, custom_ids: list, prompts: pd.Series) -> Path:
    jsonl_path = BASE_DIR / "outputs" / f"stage2m_chunk{chunk_index}_requests.jsonl"
...
def retry_failed_requests(client, chunk_index: int, failed_records: list, max_retries: int = 3) -> pd.DataFrame:
    ...
    프롬프트는 stage2m_request_index.parquet에서 custom_id로 바로 조회한다."""
...
def _run(client):
    ...
    mark_progress("gpt_conditions_v2_BD", {
    ...
    if all(s in terminal for s in statuses):
        ...
        req_idx = pd.read_parquet(REQUEST_INDEX_PATH)[["custom_id", "call_id", "condition", "n", "version", "acoustic"]]
        ...
        if n_failed:
            ...
            mark_progress("gpt_conditions_v2_BD", {"status": "done_with_failures", "failed_permanent": n_failed,
                                              "total": len(chunks)})
            print("ALL_CHUNKS_DONE_WITH_FAILURES")
        else:
            mark_progress("gpt_conditions_v2_BD", {"status": "done", "total": len(chunks)})
            print("\nALL_CHUNKS_COMPLETE")
