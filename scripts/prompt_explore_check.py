"""prompt_explore(v2/v3/v4, 600건) Batch 제출/확인. base/v2는 카테고리명만 출력이라
기존 normalize_gpt_label로 파싱하고, v3/v4는 <카테고리>/<불만> 태그를 파싱한다.
<카테고리> 태그가 없거나 내용이 7개 카테고리 중 하나가 아니면 "형식오류"로 기록
(최근접 매칭 시도하지 않음).
"""
...
STATE_PATH = BASE_DIR / "outputs" / "prompt_explore_chunks_state.json"
PARTIAL_DIR = BASE_DIR / "outputs" / "prompt_explore_partial"
FINAL_PATH = BASE_DIR / "outputs" / "prompt_explore_gpt_predictions.parquet"

MODEL = "gpt-4.1-mini"
PROMPT_VERSION = "prompt_explore"
PROJECT_TAG = "prompt_explore_v1"
MAX_RETRIES = 2
...
def parse_response(raw: str, version: str):
    ...
    if version in ("v3", "v4"):
        ...
        # 카테고리 정의 자체가 "1. 환불요청" 식 번호매김이라 모델이 태그 안에도
        # 번호를 그대로 echo하는 경우가 있음 - base/v2와 동일하게 앞머리 번호만
        # 제거(정규화)하고 비교. 이건 "최근접 매칭"이 아니라 표기 정규화임.
        ...
        try:
            return normalize_gpt_label(raw), None, None
        except Exception as e:
            return None, f"파싱오류({e})", None
...
def submit_chunk(client, chunk_index: int, plan: list, prompts: pd.Series):
    ...
    jsonl_path = BASE_DIR / "outputs" / f"prompt_explore_chunk{chunk_index}_requests.jsonl"
...
def retry_failed_requests(client, chunk_index: int, failed_records: list, versions: pd.Series,
    ...
    for custom_id, err_msg in failed_records:
        version = versions.loc[custom_id] if custom_id in versions.index else "base"
