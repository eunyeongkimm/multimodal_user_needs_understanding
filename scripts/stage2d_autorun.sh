#!/bin/bash
# stage2d_check.py를 반복 폴링하며 동시-제출 파이프라인(최대 8개 안팎 청크
# 동시 in-flight, 완료되는 대로 즉시 backfill)을 굴린다. 모든 청크가
# 터미널 상태에 도달하면 자동으로 stage2e_analysis.py(비교표 출력)까지 실행한다.

cd "$(dirname "$0")/.." || exit 1
source venv/bin/activate

LOG="outputs/stage2d_autorun.log"
POLL_INTERVAL=90  # seconds - 동시 여러 청크가 끝날 수 있어 기존보다 짧게

echo "$(date '+%Y-%m-%d %H:%M:%S') stage2d autorun started (pid $$)" >> "$LOG"

while true; do
  OUT=$(python3 scripts/stage2d_check.py 2>&1)
  echo "$(date '+%Y-%m-%d %H:%M:%S') $OUT" >> "$LOG"

  if echo "$OUT" | grep -qE "ALL_CHUNKS_COMPLETE|ALL_CHUNKS_DONE_WITH_FAILURES"; then
    echo "$(date '+%Y-%m-%d %H:%M:%S') GPT 조건 실행 완료 - 분석 단계 자동 진행" >> "$LOG"

    echo "$(date '+%Y-%m-%d %H:%M:%S') === stage2e_analysis.py 시작 ===" >> "$LOG"
    python3 scripts/stage2e_analysis.py >> "$LOG" 2>&1
    if [ $? -ne 0 ]; then
